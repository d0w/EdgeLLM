...
import (
	...
	"github.com/d0w/EdgeLLM/internal/client"
...
func runDistributedInference(cmd *cobra.Command, args []string) error {
	...
	slog.Info("Starting distributed inference...")
	slog.Info(fmt.Sprintf("Model: %s", inferenceModel))
	// slog.Info(fmt.Sprintf("Prompt: %s", inferencePrompt))
	// slog.Info(fmt.Sprintf("Strategy: %s", inferenceStrategy))
	...
	// 1. Create client
	// 2. Find worker nodes
	// 3. Create cluster with found nodes
	// 4. Negotiate tunnel for vllm server head

	inferenceClient := client.NewClient("127.0.0.1:60051", inferenceModel)
	...
	if err := inferenceClient.Chat(); err != nil {
		return fmt.Errorf("inference chat failed: %w", err)
	}
